...
import { useGoldrushStats } from '@/app/(control-panel)/goldrush/api/hooks/useGoldrushStats';
import { formatMoney } from '@/lib/money';
import type { LedgerItem, Player } from '@/app/(control-panel)/goldrush/api/types';
...
function GoldrushDashboardView() {
	const theme = useTheme();
	const { data: stats, isLoading, isError, error, refetch } = useGoldrushStats();
	...
	if (isError || !stats) {
		return (
			<div className="flex h-full flex-col items-center justify-center gap-4 p-8">
				<Typography variant="h5">Could not load Goldrush data</Typography>
				<Typography color="text.secondary">
					{error instanceof Error ? error.message : 'Check that the admin API can reach the Goldrush database.'}
				</Typography>
				<Button
					variant="contained"
					color="secondary"
